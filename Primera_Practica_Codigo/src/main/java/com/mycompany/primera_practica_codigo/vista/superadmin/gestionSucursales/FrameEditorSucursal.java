...
public class FrameEditorSucursal extends javax.swing.JFrame {
    ...
    private void recolectarDatosSucursal() {
        ...
        try {
            nombreSucursal = recolector.recolectarTexto(jTextFieldNombreSucursal);
        } catch (TextoVacioException ex) {
            mensajeError = "El nombre de la sucursal esta vacio";
            mostrarMensajeErrorRecolector(mensajeError);
        }

        try {
            ubicacion = recolector.recolectarTexto(jTextFieldUbicacion);
        } catch (TextoVacioException ex) {
            mensajeError = "La ubicacion no puede estar vacia";
            mostrarMensajeErrorRecolector(mensajeError);
        }

        try {
            telefono = recolector.recolectarTexto(jTextFieldTelefono);
        } catch (TextoVacioException ex) {
            mensajeError = "El telefono de la sucursal no debe estar vacia";
            mostrarMensajeErrorRecolector(mensajeError);
        }

    }

    private void validarDatosSucursal() {
        errorEnValidacion = false;
        VerificadorDatosSucursal verificadorDatosSucursal = new VerificadorDatosSucursal();
        try {
            verificadorDatosSucursal.verificarDatos(nombreSucursal);
            if (verificadorDatosSucursal.getExisteSucursal() && !esEdicion) {
                errorEnValidacion = true;
                String mensajeError = "Ya existe una sucursal con ese nombre, por favor elija otro";
